        ...
        if(resultBitmap != null) {
            ...
            if(isNetworkConnected(context)) {
                ...
                if(currentPossible.isEmpty()) {
                    ...
                    paint.setColor(resultPointColor);
                    for(ResultPoint point : currentPossible) {
                        canvas.drawCircle(frame.left + point.getX(), frame.top + point.getY(),
                                6.0f, paint);
                    }
                }
                if(currentLast != null) {
...
//                // canvas.drawText("当前网络不可用，清检测网络设置", frame.left,
//                // (float)(frame.bottom - (float)TEXT_PADDING_TOP * density), paint);
//
//
//
//                String pageTitle = "当前网络不可用，请检测网络设置";
//
//                RectF bounds = new RectF(frame);
//                // measure text width
//                bounds.right = paint.measureText(pageTitle, 0, pageTitle.length());
//                // measure text height
//                bounds.bottom = paint.descent() - paint.ascent();
//
//                bounds.left += (frame.width() - bounds.right) / 2.0f;
//                bounds.top += (frame.height() - bounds.bottom) / 2.0f;
//
//                paint.setColor(Color.WHITE);
//                paint.setTextSize(TEXT_SIZE * density);
//                canvas.drawText(pageTitle, bounds.left,
//                        (float)(frame.bottom - (float)TEXT_PADDING_TOP * density), paint);
//            }
            // 只刷新扫描框的内容，其他地方不刷新
            postInvalidateDelayed(ANIMATION_DELAY, frame.left, frame.top, frame.right, frame.bottom);
